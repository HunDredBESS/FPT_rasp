...
LINE_ACCESS_TOKEN = "JQhCYNB3078eklmRN732OCAtmX2y8YItxYI9XefiguG"
URL_LINE = "https://notify-api.line.me/api/notify" 

path_csv = r"C:\Users\roika\Documents\Project_Arduino\FingerprintAndImage\Data_csv\Data_testRev02.csv"
...
def ConvertTointlist(hex_tuples_list):
     # Convert each hexadecimal string to a list of integers
    # result_list = [
    #     [int(hex_value, 0) for hex_value in hex_string.split(',')] 
    #     for hex_string in hex_strings_list
    # ]
    result_list = [
        [int(hex_value, 0) for hex_value in hex_tuple] 
        for hex_tuple in hex_tuples_list
    ]
    flattened_list = [value for sublist in result_list for value in sublist]

    return flattened_list

def as608_template_matching(template1, template2):
    # Normalized cross-correlation (NCC)
    """
    Matches the templates of two AS608 fingerprint sensor readings.

    Args:
        template1: The first AS608 fingerprint sensor template.
        template2: The second AS608 fingerprint sensor template.

    Returns:
        A score indicating the similarity of the two fingerprint templates.
    """

    # Calculate the normalized cross-correlation between the templates
    # correlation = np.corrcoef(template1.reshape(-1), template2.reshape(-1))[0, 1]

    # Convert template1 to a NumPy array before calculating the correlation
        ...
        # try:
            ...
            if (conditio_S=='Login'):
                ...
                if (ID_Data!=0):
                    ...
                    Scan_FingerPrintTemplate_List = ConvertTointlist(Scan_FingerPrintTemplate)
                    similarity_score2 = as608_template_matching(minutiae1,Scan_FingerPrintTemplate_List)
                    print(f"Similarity score: {similarity_score2}")
                    # print(Scan_FingerPrintTemplate_List)
                    # get the current date and time
                    now = datetime.datetime.now()
                    line_text("\nName: "+Name_Data+"\nStudent ID: "+StudentID_data+f"\ndate: +{str(now)}"+f"\nSimilarity score: {similarity_score2}")
